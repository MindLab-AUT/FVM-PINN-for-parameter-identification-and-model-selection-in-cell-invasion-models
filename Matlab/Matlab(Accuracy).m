...
for n = 1:N_t
    ...
    U_pinn_total((n - 1) * N_point + 1:n * N_point) = U_pinn;
end

U_mle_total = zeros(N_point*N_t,1);
for n = 1:N_t
    D_U_mle = D_mle(U_mle);
    ...
    % Solve the linear system for the next time step
    U_mle = A_left \ b_mle;
    U_mle_total((n - 1) * N_point + 1:n * N_point) = U_mle;
end
% Plot at final time
figure
...
U_mle_total = reshape(U_mle_total,nx*ny*77,1);
time = zeros(77,1);
% Compute error
for i = 1:N_t
    time(i) = i;
    err_pinn(i)=sqrt((sum((U_pinn_total((i - 1) * N_point + 1:i * N_point)-U_total((i - 1) * N_point + 1:i * N_point)).^2))/N_point);
    err_mle(i)=sqrt(sum((U_mle_total((i - 1) * N_point + 1:i * N_point)-U_total((i - 1) * N_point + 1:i * N_point)).^2)/N_point);
end
time = reshape(time,1,77);
plot(time,err_pinn,time,err_mle);
legend('pinn', 'mle')
title('Standard Fisher Model')
xlabel('Time Step');
ylabel('Error');
print('error_Standard1',  '-depsc');

% Calculate AIC & BIC
n = N_point*N_t;        % Number of data points
k = 944; % Number of parameters in the Neural Network
