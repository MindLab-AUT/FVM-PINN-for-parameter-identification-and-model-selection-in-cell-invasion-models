...
scale_r = 0.1;
scale_K = 1000;
scale_eta = 0.01;  % Scaling factor for eta

% Scale initial guesses
initial_guess_scaled = [D0 / scale_D0, r / scale_r, K / scale_K, eta / scale_eta];
lb_scaled = [100 / scale_D0, 0.01 / scale_r, 500 / scale_K, 0 / scale_eta];
ub_scaled = [10000 / scale_D0, 1 / scale_r, 5000 / scale_K, 3 / scale_eta];

% Set optimization options
options = optimoptions('fmincon', ...
    'Display', 'iter', ...
    'Algorithm', 'sqp', ...
    'MaxIterations', 1, ...  % Limit to 10 iterations
    'OptimalityTolerance', 1e-6, ... % Fine-tune the tolerance
    'StepTolerance', 1e-6);    % Fine-tune step size tolerance

% Optimize scaled parameters (including eta)
optimal_params_scaled = fmincon(@(params_scaled) cost_function_scaled(params_scaled, U_obs, ...
    nx, ny, L_right, T_steps, scale_D0, scale_r, scale_K, scale_eta, dx, dy, dt), ...
...
optimal_params = [optimal_params_scaled(1) * scale_D0, ...
                  ...
                  optimal_params_scaled(4) * scale_eta];
...
disp(['eta: ', num2str(optimal_params(4))]);
