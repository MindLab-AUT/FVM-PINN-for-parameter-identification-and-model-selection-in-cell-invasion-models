...
% Scale initial value
initial_guess_scaled = [D0 / scale_D0, r / scale_r, K / scale_K];
lb_scaled = [100 / scale_D0, 0.01 / scale_r, 500 / scale_K];
ub_scaled = [10000 / scale_D0, 1 / scale_r, 5000 / scale_K];

% Set optimization options
options = optimoptions('fmincon', ...
    'Display', 'iter', ...
    'Algorithm', 'interior-point', ...
    'MaxIterations', 10, ...  % Limit to 5 iterations
...
optimal_params_scaled = fmincon(@(params_scaled) cost_function_scaled(params_scaled, U_obs, ...
    nx, ny, L_right, T_steps, scale_D0, scale_r, scale_K, dx, dy, dt), ...
...
function error = cost_function_scaled(params_scaled, U_obs, nx, ny, L_right, T_steps, ...
                                       scale_D0, scale_r, scale_K, dx, dy, dt)
    ...
    % Compute error
    error = sqrt((sum(((U_obs - U_sim)).^2))/(nx*ny));
end
